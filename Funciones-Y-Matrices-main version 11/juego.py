...
class Juego:
    ...
    def dibujar_pantalla_inicio(self):
        ...
        titulo = pygame.font.SysFont(None, 48).render("Aventura optimizada", True, WHITE)
        subtitulo = self.font.render("Presiona Enter para jugar", True, YELLOW)
        t_rect = titulo.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 - 30))
        s_rect = subtitulo.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 + 20))
        self.screen.blit(titulo, t_rect)
        self.screen.blit(subtitulo, s_rect)
        pygame.display.flip()

    def dibujar_pantalla_gameover(self):
        self.screen.fill(BLACK)
        titulo = pygame.font.SysFont(None, 64).render("GAME OVER", True, RED)
        puntaje_final = self.font.render(f"Puntaje: {self.score_total}", True, WHITE)
        subtitulo = self.font.render("Presiona Enter para volver al inicio", True, YELLOW)
        t_rect = titulo.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 - 50))
        p_rect = puntaje_final.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2))
        s_rect = subtitulo.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 + 40))
        self.screen.blit(titulo, t_rect)
        self.screen.blit(puntaje_final, p_rect)
        self.screen.blit(subtitulo, s_rect)
        pygame.display.flip()
